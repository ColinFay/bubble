...
`NodeSession$new()` tries to guess where NodeJS is by looking at both these `Sys`. If they are both empty, you'll get an error, and need to provide the path manually.

This function returns an object that can be used to interact with the launched Node session.
